 ...
 */
final class Version20210308132758 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, username, is_active, is_blocked, roles, password FROM user');
        $this->addSql('DROP TABLE user');
        $this->addSql('CREATE TABLE user (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username VARCHAR(255) NOT NULL COLLATE BINARY, is_active BOOLEAN NOT NULL, is_blocked BOOLEAN NOT NULL, password VARCHAR(255) NOT NULL COLLATE BINARY, roles CLOB DEFAULT NULL --(DC2Type:json)
        , email VARCHAR(255) NOT NULL, is_verified BOOLEAN NOT NULL)');
        $this->addSql('INSERT INTO user (id, username, is_active, is_blocked, roles, password) SELECT id, username, is_active, is_blocked, roles, password FROM __temp__user');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__user AS SELECT id, username, is_active, is_blocked, roles, password FROM "user"');
        $this->addSql('DROP TABLE "user"');
        $this->addSql('CREATE TABLE "user" (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, username VARCHAR(255) NOT NULL, is_active BOOLEAN NOT NULL, is_blocked BOOLEAN NOT NULL, password VARCHAR(255) NOT NULL, roles CLOB DEFAULT NULL COLLATE BINARY)');
        $this->addSql('INSERT INTO "user" (id, username, is_active, is_blocked, roles, password) SELECT id, username, is_active, is_blocked, roles, password FROM __temp__user');
        $this->addSql('DROP TABLE __temp__user');
    }
}
